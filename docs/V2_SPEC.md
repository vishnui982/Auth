...
`authority.delegate` carries the complete child grant and uses its target scope as the Action resource. An explicit denial of the underlying operation or delegation operation prevents delegating any overlapping authority. The check covers both a denied ancestor and a denied descendant inside a proposed prefix grant. Because grants cannot encode exceptions, such a broad delegation is rejected; a disjoint narrower grant can still succeed. Denials are principal-specific policy restrictions, not annotations inherited by every recipient. `authority.revoke` targets a grant ID at a registered authority endpoint. Administrators, the grant's issuer, or its subject may revoke it; unrelated principals may not. Root revocation invalidates every descendant because each use rechecks the entire chain. Revocation changes the state commitment, invalidating outstanding certificates as well.
...
The pure `transition` function handles state evolution separately from authorization. A successful transition increments step and the operation count, then applies the effect-specific changes. It validates observed read/result commitments using canonical JSON equality, so JSON `true` cannot substitute for the integer `1`. Policy-relative structural validation requires the configured roots, well-formed attenuated grant chains, known labels/principals/operations, the exact object catalog, retained baseline object labels, known revocations and a total operation count equal to `step`. Expired and revoked grants remain structurally valid records but cannot authorize use. Offline verification recomputes this entire transition and rejects an incorrect next state even when someone has signed it with the correct key.
...
An execution event refers to the exact authorization envelope, binds a result hash, and is `executed` for local effects or `queued` for a send intent. Its state is computed from the current snapshot and the actual local result. The resource checks signature, source/policy/domain pins, actor/action equality, issuance record, unused nonce, current state, expiry and current semantics. Grant expiry during the certificate's 30-second window still blocks execution. Event timestamps must be nondecreasing; a wall-clock rollback behind the last recorded event blocks new authorization/execution until the clock catches up. Historical verification checks the same ordering. This does not replace a trustworthy clock or an independent rollback anchor.
...
At startup the service compares database trust metadata against the separately supplied trust record, replays the full signed history, and compares the resulting state to the stored snapshot. It also verifies private object value commitments, the outbox commitment and materialized nonce consumption records. It refuses inconsistent state. Before each operation and export it also authenticates the ledger, checks database sequence/hash columns against the signed records, checks event content bindings, compares the latest signed state to the live snapshot, and checks outbox IDs and issuance/consumption records. These checks scan the local history; this correctness-oriented version does not claim constant-time request cost as the history grows.
...
Signatures use a unique base64 encoding; alternate encodings of identical signature bytes are rejected to keep envelope commitments unambiguous.
